...
class PoseFeatureRepository(PoseFeatureInterface):
    ...
    def calculate_stability(self, keypoints: KeypointCollection, 
        ...
        """Delegate to service implementation."""
        return self._service.calculate_stability(keypoints, previous_keypoints, window_size)
    ...
    @staticmethod
    def convert_to_keypoint_collection(keypoints_dict: Dict[str, Tuple[float, float, float]]) -> KeypointCollection:
        """
        ...
        for joint_name, (x, y, conf) in keypoints_dict.items():
            keypoints[joint_name] = Keypoint(x=float(x), y=float(y), confidence=float(conf))
    ...
    def visualize_keypoints(self, frame: np.ndarray, 
        ...
        for joint_name, keypoint in keypoints.keypoints.items():
            if keypoint.confidence > threshold:
                ...
                cv2.putText(frame_copy, joint_name, (x, y), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
        
        return frame_copy
    
    def visualize_angles(self, frame: np.ndarray, 
                      keypoints: KeypointCollection, 
        ...
        for name, keypoint in keypoints.keypoints.items():
            if keypoint.confidence > confidence_threshold:
                cv2.circle(annotated_frame, (int(keypoint.x), int(keypoint.y)), 4, (0, 255, 0), -1)
