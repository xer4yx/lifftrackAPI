...
class FrameRepository(FrameRepositoryInterface):
        ...
        try:
            ...
            try:
                if 0.49 <= uv_ratio <= 0.51:
                    ...
                    u = np.frombuffer(u_plane_bytes, dtype=np.uint8)[
                        : uv_width * uv_height
                    ].reshape((uv_height, uv_width))
                    v = np.frombuffer(v_plane_bytes, dtype=np.uint8)[
                        : uv_width * uv_height
                    ].reshape((uv_height, uv_width))
                    ...
                    try:
                        ...
                        return (
                            cv2.rotate(nv12, cv2.ROTATE_90_CLOCKWISE)
                            if width > height
                            else nv12
                        )
                        ...
                        try:
                            ...
                            i420[0:height, :] = y
                            i420[height : height + height // 4, :width] = cv2.resize(
                                u, (width, height // 4)
                            )
                            i420[height + height // 4 :, :width] = cv2.resize(
                                v, (width, height // 4)
                            )
                            ...
                            return (
                                cv2.rotate(i420, cv2.ROTATE_90_CLOCKWISE)
                                if width > height
                                else i420
                            )
                            ...
                            try:
                                ...
                                if (
                                    width > height
                                ):  # If image is in landscape but should be portrait
                                    nv12 = cv2.rotate(nv12, cv2.ROTATE_90_CLOCKWISE)
                                logger.info(
                                    "Successfully converted YUV to BGR using direct conversion"
                                )
                                return nv12
            ...
            if width > height:
                ...
                )  # Swapped dimensions for portrait
    ...
    async def parse_frame(self, byte_data: bytes) -> Optional[Any]:
        ...
        try:
            ...
            v_plane = byte_data[
                data_start + y_size + u_size : data_start + y_size + u_size + v_size
            ]
