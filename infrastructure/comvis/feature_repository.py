...
class FeatureRepository(FeatureRepositoryInterface):
    ...
    def perform_frame_analysis(self, frames_buffer: List[np.ndarray], request: Request) -> Tuple[Dict, Dict, List, str]:
        """
        ...
        try:
            ...
            def dict_to_keypoint_collection(pose_dict: Dict) -> KeypointCollection:
                ...
                for joint_name, (x, y, confidence) in pose_dict.items():
                    keypoints[joint_name] = Keypoint(x=float(x), y=float(y), confidence=float(confidence))
                return KeypointCollection(keypoints=keypoints)
            
            current_keypoints = dict_to_keypoint_collection(current_pose)
            previous_keypoints = dict_to_keypoint_collection(previous_pose) if previous_pose else None
            ...
            if previous_keypoints and previous_keypoints.keypoints:
                movement_patterns = pose_service.extract_movement_patterns(current_keypoints, previous_keypoints)
                speeds = pose_service.calculate_speed(movement_patterns)
                stability = pose_service.calculate_stability(current_keypoints, previous_keypoints)
            
            # Extract body alignment
            vertical_alignment, lateral_alignment = pose_service.extract_body_alignment(current_keypoints)
            from core.entities.pose_entity import BodyAlignment
            body_alignment = BodyAlignment(vertical_alignment=vertical_alignment, lateral_alignment=lateral_alignment)
            
            # Create initial PoseFeatures object
            features = PoseFeatures(
                keypoints=current_keypoints,
                objects=object_inference_dict if isinstance(object_inference_dict, dict) else {},
    ...
    def get_suggestions(self, features: PoseFeatures, class_name: str) -> Tuple[float, str]:
        ...
        try:
            ...
            return (0.0, "Unable to analyze form at this time. Please continue your exercise.") 