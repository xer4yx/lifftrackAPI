...
class PoseNetInferenceService(InferenceInterface):
    ...
    def __init__(
        ...
        self._model_url = model_url or config.get(
            section="TensorHub", option="MOVENET_MODEL"
        )
        ...
        try:
            ...
            logger.info(
                f"Initialized PoseNet Inference Service with model: {self._model_url}"
            )
    ...
    def _preprocess_image(self, image: np.ndarray) -> tf.Tensor:
        ...
        if (
            image_hash in self._prediction_cache
            and "preprocessed" in self._prediction_cache[image_hash]
        ):
    ...
    def infer(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                image_hash in self._prediction_cache
                and "result" in self._prediction_cache[image_hash]
            ):
            ...
            return {"keypoints": keypoints, "processing_time": elapsed_time}
    ...
    def _process_keypoints(
        self, keypoints_data: np.ndarray, image_shape: Tuple[int, int, int]
    ...
    def visualize_pose(
        self, frame: np.ndarray, keypoints: Dict[str, Tuple[int, int, float]]
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            for connection in connections:
                ...
                if (
                    start_point in keypoints
                    and end_point in keypoints
                    and keypoints[start_point][2] > self._confidence_threshold
                    and keypoints[end_point][2] > self._confidence_threshold
                ):
