...
class Authenticator(AuthenticationInterface):
    ...
    async def initialize_username_cache(self):
        ...
        try:
            ...
            if users:
                ...
                self.logger.info(
                    f"Username cache initialized with {len(self.username_cache)} entries"
                )
    ...
    async def authenticate(
        ...
        try:
            ...
            if (
                not credentials.password
            ):  # For token validation when password not needed
                return user
    ...
    async def verify_token(self, token: str) -> Optional[TokenDataEntity]:
        ...
        try:
            ...
            if (
                ...
                and await self.token_blacklist_repository.is_blacklisted(token)
            ):
    ...
    async def logout(self, token: str) -> bool:
        ...
        try:
            ...
            # Add to blacklist
            return await self.token_blacklist_repository.add_to_blacklist(
                token, token_data.exp
            )
    ...
    async def validate_user_input(
        ...
        return ValidationResultEntity(is_valid=len(errors) == 0, errors=errors)
