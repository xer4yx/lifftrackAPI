...
class AuthUseCase:
    ...
    async def register_user(
        ...
        # Hash the password
        user_data["password"] = await self.auth_service.hash_password(
    ...
    async def login(
    ...
    ) -> Tuple[bool, Optional[TokenEntity], Optional[UserEntity], Optional[str]]:
        """
        ...
        token = await self.auth_service.create_token(
            user, timedelta(minutes=30)  # Use config value in actual implementation
        )
    ...
    ) -> Tuple[bool, Optional[TokenEntity], Optional[str]]:
        """
