...
class PoseFeatures(BaseModel):
    ...
    joint_angles: Dict[str, float] = Field(
        default_factory=dict, serialization_alias="joint_angles"
    )
    movement_patterns: Dict[str, float] = Field(
        default_factory=dict, serialization_alias="movement_patterns"
    )
    movement_pattern: str = Field(default="", serialization_alias="movement_pattern")
    body_alignment: Optional[BodyAlignment] = Field(
        default=None, serialization_alias="body_alignment"
    )
    ...
    form_issues: Dict[str, bool] = Field(
        default_factory=dict, serialization_alias="form_issues"
    )
    ...
    def model_dump(self, **kwargs):
        ...
        if (
            "keypoints" in data
            and isinstance(data["keypoints"], dict)
            and "keypoints" in data["keypoints"]
        ):
