...
from lifttrack.models.user_schema import (
    User,
    UserUpdate,
    UserResponse,
    UserCreateResponse,
...
async def register_user(
    user_data: User, user_service: UserUseCase = Depends(get_user_service_rest)
):
    """
...
async def get_profile(current_user: UserEntity = Depends(get_current_user)):
    ...
    return UserResponse.model_validate(
        obj=current_user.model_dump(exclude={"password"}), strict=False
    )
...
async def get_user(
    ...
    return UserResponse.model_validate(
        obj=user.model_dump(exclude={"password"}), strict=False
    )
...
async def update_user(
    ...
    success, updated_user, error = await user_service.update_user_profile(
        user_id=current_user.username, update_data=user_update_dict
    )
