...
thread_pool = concurrent.futures.ThreadPoolExecutor(max_workers=2, thread_name_prefix="ws-shared-pool")
...
async def livestream_exercise_tracking(
    ...
    token: str = Query(None, description="JWT authentication token (alternative to subprotocol auth)"),
    ...
    async def analyze_buffer(frames_buffer: List[Any], current_frame_count: int, second_number: int):
        ...
        try:
            ...
            def sequential_analysis():
                ...
                previous_frame = frames_buffer[-2] if len(frames_buffer) > 1 else frames_buffer[-1]
                
                # Process frames sequentially instead of concurrently
                # Get pose keypoints for current and previous frames
                current_pose_result = inference_service._pose_estimation.infer(current_frame)
                previous_pose_result = inference_service._pose_estimation.infer(previous_frame)
                
                # Get object detections from current frame
                objects_result = inference_service._object_detection.infer(current_frame)
                
                # Get action recognition from the full buffer
                action_result = inference_service._action_recognition.infer(frames_buffer)
                ...
                predicted_class_name = action_result.get("predicted_class_name", exercise_name)
                
                return current_pose, previous_pose, objects, predicted_class_name
            
            # Run the sequential analysis in the shared thread pool
            current_pose, previous_pose, objects, predicted_class_name = await loop.run_in_executor(
                thread_pool, sequential_analysis
            )
            ...
            accuracy, suggestions = comvis_service.get_suggestions(features, exercise_name)
            ...
            try:
                ...
                logger.error(f"Failed to save analysis for {time_frame}: {str(save_error)}")
    ...
    try:
        while connection_active and frame_count < max_frames:
            ...
            if frame is not None:
                ...
                processed_frame = await loop.run_in_executor(
                    thread_pool, 
                    lambda: comvis_service.frame_repo.process_frame(frame)[0]  # Only need the processed frame
                )
        ...
        if pending_analyses:
            try:
                ...
                if pending:
                    logger.warning(f"{len(pending)} analysis tasks did not complete in time, cancelling them")
                    for task in pending:
                        task.cancel()
