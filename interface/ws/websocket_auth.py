...
async def authenticate_websocket_query_param(
    websocket: WebSocket, token: str, username: str, auth_service: AuthUseCase
    ...
    try:
        ...
        if client.username != username:
            logger.warning(
                f"Username mismatch in WebSocket auth: {client.username} != {username}"
            )
    ...
    try:
        ...
        if not token:
            for protocol in protocols:
                if (
                    protocol not in ["Authorization", expected_subprotocol]
                    and len(protocol) > 20
                ):
        ...
        if client.username != username:
            logger.warning(
                f"Username mismatch in WebSocket subprotocol auth: {client.username} != {username}"
            )
            return False, None, "Username does not match token"

        logger.info(
            f"WebSocket authenticated successfully via subprotocol for user: {username}"
        )
        return True, client, None
