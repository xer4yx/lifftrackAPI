...
from interface.ws.websocket_router import livestream_exercise_tracking, websocket_router_v3
...
def mock_comvis_service():
    ...
    mock_service.create_frame_from_planes.return_value = np.zeros((120, 160, 3), dtype=np.uint8)
    # Return both frame and buffer to match expected return signature
    mock_service.process_frame.return_value = (np.zeros((120, 160, 3), dtype=np.uint8), {})
    ...
    mock_frame_repo.process_frame.return_value = (np.zeros((120, 160, 3), dtype=np.uint8), {})
    mock_service.frame_repo = mock_frame_repo
    
    # Add other missing methods that might be called
    mock_service.parse_frame = AsyncMock(return_value=np.zeros((120, 160, 3), dtype=np.uint8))
...
def override_dependencies(app, mock_auth_service, mock_comvis_service, mock_inference_service, mock_user, mock_token, mock_db):
...
@pytest.mark.asyncio
async def test_websocket_username_mismatch(mock_websocket, mock_auth_service, mock_user, mock_db):
...
@pytest.mark.asyncio
async def test_websocket_invalid_subprotocol(mock_websocket, mock_auth_service, mock_user, mock_db):
...
@pytest.mark.asyncio
async def test_websocket_basic_connection_and_messaging(app, mock_auth_service, mock_user):
    ...
    # Apply the patch
    with patch("interface.ws.websocket_router.livestream_exercise_tracking", mock_endpoint):
...
async def test_websocket_frame_processing(
    mock_websocket, mock_auth_service, mock_user, mock_comvis_service, mock_inference_service, mock_db
):
...
async def test_websocket_analyze_buffer(
    ...
    receive_sequence = [frame_data] * 30 + [b'COMPLETED'] * 10  # Add extra COMPLETED signals
    ...
    try:
        ...
        assert pose_estimation_called, "Pose estimation should be called during frame analysis"
        assert object_detection_called, "Object detection should be called during frame analysis"
        assert action_recognition_called, "Action recognition should be called during frame analysis"
