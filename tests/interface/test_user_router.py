...
async def test_register_user_failure(client, mock_user_service):
    ...
    mock_user_service.register_user.return_value = (False, None, "Username already exists")
...
async def test_get_profile_success(client, mock_current_user):
    # Act
    with patch("interface.routers.user_router.get_current_user", return_value=mock_current_user):
...
async def test_get_user_success(client, mock_user_service, mock_current_user):
    ...
    # Act
    with patch("interface.routers.user_router.get_user_service_rest", return_value=mock_user_service):
        with patch("interface.routers.user_router.get_current_user", return_value=mock_current_user):
...
@pytest.mark.asyncio
async def test_update_user_password_rejected(client, mock_user_service, mock_current_user):
    # Arrange
    user_id = "test_user_id"  # Same as mock_current_user.id
...
async def test_update_user_failure(client, mock_user_service, mock_current_user):
    ...
    # Setup mock to return failure
    mock_user_service.update_user_profile.return_value = (False, None, "Invalid email format")
...
async def test_delete_user_failure(client, mock_user_service, mock_current_user):
    ...
    # Setup mock to return failure
    mock_user_service.delete_user.return_value = (False, "Failed to delete user from database")
...
@pytest.mark.asyncio
async def test_change_password_incorrect_old_password(client, mock_user_service, mock_current_user):
    # Arrange
    user_id = "test_user_id"  # Same as mock_current_user.id
    
    # Setup mock to return failure
    mock_user_service.change_user_password.return_value = (False, "Current password is incorrect")
    ...
    with patch("interface.routers.user_router.get_user_service_rest", return_value=mock_user_service):
        with patch("interface.routers.user_router.get_current_user", return_value=mock_current_user):
            response = client.post(
                f"/v3/user/{user_id}/change-password",
                params={"old_password": "wrong_password", "new_password": "new_password"}
            )
